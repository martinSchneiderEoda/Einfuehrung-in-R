...
- **Vektoren** (eindimensionales Array): kann nummerische, Zeichen oder logische Werte enthalten. Die Elemente in einem Vektor haben alle die gleichen Datentypen.
- **Matrizen** (zweidimensionales Array): kann nummerische, Zeichen oder logische Werte enthalten. Die Elemente in einer Matrix haben alle die gleichen Datentypen.
- **Dataframes** (zweidimensionale Objekte): kann nummerische, Zeichen oder logische Werte enthalten. Innerhalb einer Spalte haben alle Elemente den gleichen Datentyp, aber verschiedene Spalten können von unterschiedlichen Datentypen sein.
...
- moviename: eine Zeichenkette mit den Filmtiteln (gespeichert in `mov`)
- actors: ein Vektor mit den Namen der wichtigsten Schauspiel und Schauspielrinnen (gespeichert in `act`)
- reviews: ein Dataframe, der einige Bewertungen (gespeichert in `rev`) enthält
...
## Das Hinzufügen weiterer Filminformationen in die Liste
...
Dieser Befehl erweitert einfach die Originalliste, `my_list` mit der Komponente `my_val`. Diese Komponente wird an das Ende der Liste angehängt. Wenn Sie dem neuen Listeneintrag einen Namen geben wollen, fügen Sie einfach den Namen wie zuvor ein:
