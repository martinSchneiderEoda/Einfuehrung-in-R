...
*** =hint
Lesen Sie die Aufgabe sorgfältig, denn die Antwort ist darin inhalten!
...
Das Wichtigste für einen Film, der eine Legende in Hollywood werden will, sind die weltweiten Einnahmen an den Kinokassen.
...
## Hinzufügen einer Spalte für die weltweiten Kinokassen

In der vorherigen Übung berechneten Sie die Vektoren, die die Belege für die weltweite Kassenzahlen für jeden der drei Star Wars-Filme enthielten. Jedoch ist dieser Vektor noch kein Teil von `star_wars_matrix`.
...
*** =hint
In dieser Übung sollen Sie die zwei Variablen `star_wars_matrix` und `worldwide_vector` mit Hilfe des `cbind()`-Befehls zusammenfügen. Weisen Sie das Ergebnis der `all_wars_matrix` zu.
...
Als neu eingestellter Datenanalyst für Lucasfilm ist es Ihre Aufgabe herauszufinden, wie viele Besucher jedes Films aus welchen geografischen Bereichen kamen. Sie haben schon die Gesamteinnahmen in `all_wars_matrix`. Angenommen der Preis pro Ticket lag bei 5 Dollar. Teilen Sie einfach die Kasseneinnahmen durch die Ticketpreise, dann ergibt sich die Anzahl der Besucher.
