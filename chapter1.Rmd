...
description : "In diesem Kapitel machen Sie Ihre ersten Schritte mit R. Sie erfahren, wie man die Konsole als Taschenrechner verwendet und wie man Variablen zuweist. Außerdem lernen Sie die grundlegenden Datentypen in R kennen. Los geht's!!"
...
success_msg("Ausgezeichnet! Sehen Sie, wie die Konsole das Ergebnis des R-Codes anzeigt? Nun sind Sie mit den Grundzügen vertraut und wir machen uns an die Arbeit mit R!")
```
...
*** =sample_code
```{r}
# Weisen Sie der Variable my_apples den Wert 5 zu
...
*** =instructions
- Weisen Sie der Variable `my_oranges` den Wert 6 zu.
- Addieren Sie die Variablen `my_apples` und `my_oranges`. R gibt Ihnen ein einfaches Ergebnis aus.
- Weisen Sie dem Ergebnis der Addition von `my_apples` und `my_oranges` eine neue Variable `my_fruit` zu.
...
*** =sample_code
```{r}
# Weisen Sie die Werte der einzelnen Variablen my_apples und my_oranges zu
my_apples <- 5
...
success_msg("Schön! Ein großer Vorteil beim Arbeiten mit Variablen ist die Wiederverwendbarkeit. Wenn Sie für `my_apples` statt dn Wert 5 den Wert 12 zuordnen und Ihr R-Skript wiederholen, dann aktualisiert sich Ihre Variable `my_fruit` automatisch. Machen Sie mit der nächsten Übung weiter.")
```
...
*** =hint
Sie haben den numerischen Wert `6` der Variable `my_oranges` anstelle des Zeichenwertes `"six"` zugeordnet. Bitte achten Sie auf die Verwendung der Anführungszeichen, um zu zeigen, dass `"six"` ein Zeichen ist.
...
*** =solution
```{r}
# Weisen Sie der Variable my_apples einen Wert zu
my_apples <- 5  
...
success_msg("Sehr schön, Sie machen große Fortschritte. Fahren die mit der nächsten Übung fort.")
```
...
`my_numeric <- 42` , um der Variable `my_numeric` den Wert 42 zuzuweisen. 
...
*** =instructions
Vervollständigen Sie den Code im Editor und geben Sie die Datentypen von `my_character` und `my_logical` aus. 

*** =hint
Der Code, der den Datentyp von `my_numeric` ausgibt, ist bereits eingefügt. Machen Sie das Gleiche für `my_character` und `my_logical`. 
